...
(function(f,Y){async function qa(h,q,t,g,b,e,c,a){var d;try{var m=await Deno.open(t);var l=await m.stat()}catch(k){m=l=null}if(d=l&&l.isFile&&l.size){h[1][h[2]]&&(h[1][h[2]]=null);h=t.substr(t.lastIndexOf(".")+1);t=b["if-modified-since"];b=f.mime.types[h]||f.mime.defaultType;h=t&&(new Date(t)).getTime();t="1.1"===g;e=(new Date).toUTCString();c=l.mtime;if(h===c.getTime())q[t?"write":"end"]("HTTP/"+g+" 304 Not Modified\r\nDate: "+e+"\r\nServer: "+f.server+"\r\nCache-Control: max-age=864000\r\nLast-Modified: "+
...
l,!1),!0;0===d&&m.close();try{m=await Deno.open(t+c),l=await m.stat()}catch(k){m=l=null}if(d=l&&l.isFile&&l.size)return h[0](m,l,!0),!0;0===d&&m.close();h[6].error("No such file(1).",404);return!0}async function ra(h,q,t){var g=this[3],b=this[1],e=this[2],c=this[6],a=this[5],d=this[7],m=b[4],l=m[e],k;if(!h)try{h=await Deno.open(e+g),q=await h.stat()}catch(n){h=q=null}if(!t){var p=q&&q.isFile&&q.size?q.mtime.getTime():0;if(l&&p===l.ts[0])return l.ck=!1,l.ts[1]=(new Date).getTime(),a(l,c),h&&h.close(),
!0;if(p){try{await h.read(k=new Uint8Array(q.size)),l=m[e]=this[8](Z.decode(k),d,aa(e+g)),l.ts=[p,(new Date).getTime()],c._r[6]=!1,a(l,c)}catch(n){c.error("Updating script failed(1).",500)}h.close();return!0}}h&&h.close();l=m[e]={cs:b[16]};l.ts=[p,(new Date).getTime()];c._r[6]=!1;c.flush();return!0}async function sa(){var h=this[3],q=this[1],t=this[2],g=this[6],b=this[5],e=this[7],c=q[4],a=c[t];try{var d=await fetch(t+h)}catch(m){return g.error("NCS network error.")}if(h=d.ok&&(d.headers.get("Last-Modified")||
1)||0){if(a&&h===a.ts[0])a.ck=!1,a.ts[1]=(new Date).getTime(),b(a,g);else try{a=c[t]=this[8](await d.text(),e),a.ts=[h,(new Date).getTime()],g._r[6]=!1,b(a,g)}catch(m){g.error("Updating script failed(2).",500)}return!0}this[0]=ha;b=await this[0]();b.fn?(a=c[t]={cs:q[16]},a.ts=[h,(new Date).getTime()],g._r[6]=!1,g.flush()):b.error?g.error("View renderring error.\n"+b.error,500):g.error("View file not exists.",404);return!0}async function ta(h,q){var t=this[1],g=this[6];h=h||g._r[5];g=t[18];var b,e;
...
e.ok&&(e.headers.get("Last-Modified")||1)||0;if((g=b[h])&&c===g.mtime)return q&&("function"===typeof q?q(g):q[0](g)),!0;if(c){g={mtime:c};try{g.fn=t[19](await e.text()),b[h]=g}catch(a){g.error=a.stack}}else g={};q&&("function"===typeof q?q(g):q[0](g));return!0}function ua(){function h(r,u,v,z){var B=w,F=B[0],y=u[1],E=y.indexOf("?"),I=~E&&y.substr(E+1);y=~E&&y.substr(0,E)||y;E=v.host||"*";var L;if(!(L=B[9][E])){L=E.indexOf(":");var J=~L&&E.substr(0,L)||E;L=J.substr(0,J.indexOf("."));J=w[9][J]||w[9][J.replace(L,
"*")]||w[9]["*"];~J.indexOf("*")&&(J=J.replace(G,L));D===A?(A=0,w[9]={},w[9][E]=J):D>A++&&(w[9][E]=J);L=J}E=L;var M=E.indexOf(" ");L=0===M?"":E.substring(M+1);J=B[15];var K=J.length,U=B[1],O=(new Date).getTime(),N=u[2].substr(5);E=0===M?E.trimLeft():E.substring(0,M);this._args=[u,v,z];if(~y.indexOf("%"))try{y=decodeURIComponent(y)}catch(V){return this._r=C(r,"",N),this.error("Invalid unicode character in URL.",400)}"/"===y.charAt(y.length-1)&&(y=y+"index"+J);u=y.slice(-K);if(B[2]&&F._defend(F.getIP(v),
B[2]))return this._r=C(r,"",N),this.error("Block settings taking effect to prevent insecurity access.",404);K=(u=J===u)?y.substr(0,y.length-K):y;M=E+K;if(I){v.querystring=I;try{var Q=B[5](I,"&")}catch(V){f.log("HTTP QUERYSTRING parsing error: \n"+I+"\n"+V.stack,"e"),Q=B[10]}}if(v.cookie)try{var R=B[5](v.cookie,"; ")}catch(V){f.log("HTTP COOKIE parsing error: \n"+v.cookie+"\n"+V.stack,"e"),R=B[10]}if(z&&fa(z))if(v["content-type"]&&~v["content-type"].indexOf("application/dns-message"))var P={dns:z};
else try{P=Z.decode(z),P="{"===P.charAt(0)||"["===P.charAt(0)?JSON.parse(P):B[5](P)}catch(V){P=z}z=[Q||B[10],R||B[10],P||B[10],v];this._r=[r,"",N,O,K,M+B[1],!0,{suki:F,_server:v,_get:z[0],_post:z[2],_cookie:z[1]},!1,!0,!0,[null,B,M,J,U,q,this,L||E,n]];if(F=B[4][M]){z=F.ts;if(F.ck||B[3]>O-z[1]){q(F,this);return}F.ck=!0}this._r[9]=!1;if("/"===y.slice(-1)||~y.replace("\\","..").indexOf("..")||~y.indexOf(".sjs."))this.error("Please examine the config of WebServer to prevent this issue of security(1).",
...
function(b,e,c){return e+"\\\\"+c},/^[a-zA-Z0-9_$]+$/,/s\+='';/g,/^[\s]*?=[\s]*?/];"Math JSON encodeURI encodeURIComponent decodeURI decodeURIComponent escape unescape eval isNaN isFinite parseInt parseFloat null undefined NaN Infinity Object Array Function Boolean String Number Date RegExp Error".split(" ").forEach(h);"break case catch continue debugger default delete do else false finally for function if in instanceof new return switch this throw true try typeof var void while with const".split(" ").forEach(h);
...
m=a[27].exec(b);(m&&=b.indexOf(m[0]))&&(b=b.substr(0,m)+b.substr(m).replace(a[5],a[16]));b=b.replace(a[4],"");(m=a[26].exec(b))&&(m=b.indexOf(m[0]))&&(b=b.substr(0,m)+b.substr(m).replace(a[13],a[17]));m=b.indexOf("\x3c!--");~m&&(b=b.substr(0,m)+b.substr(m).replace(a[6],"").replace(a[21],"\x3c!--").replace(a[22],"--\x3e"));b=b.split("'").join("\\'").replace(a[7],a[18]).replace(a[8],function(l,k,p){k=p.replace(a[9],"'");k=k.trim();l="="===k.charAt(0)||a[29].test(k)?"';s+="+k.replace(a[31],"").replace(a[0],
...
"number"===typeof v&&(Infinity===v||/^[0-9]+$/.test(v.toString()))&&W("checkInterval",1E3*v);n._aliasHostHeader&&W("aliasHostHeader",n._aliasHostHeader);f.version||(f.version="suki.js_v11.0.17");"port"===x&&(ca.listen(w,n.bind||"::"),ca.parse=oa,f.log(f.dateFormat(new Date)+" UTC\ndeno_v"+Deno.version.deno.replace("v","")+"\n"+f.version+" Javascript Application Server (HTTP listening on "+(n.bind||"::")+":"+w+")  \r\n"));return q}}function q(n){n(f)}function t(n){function w(G){var C="",r="",u="undefined"===
