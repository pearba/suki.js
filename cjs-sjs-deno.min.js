...
mng:"video/x-mng",asx:"video/x-ms-asf",asf:"video/x-ms-asf",wmv:"video/x-ms-wmv",avi:"video/x-msvideo"},defaultType:"application/octet-stream"};var da,W,na,oa,la,ja;f.server="suki";f.extention=".sjs";f.moduleExtention=".sjs.js";f.nullObj=Object.freeze({});f.nullFunc=function(){}.bind(f.nullObj);f.platform=Y.os.platform();f.strlen=Buffer.byteLength;f.isLinux="linux"===f.platform;f.isWindows="win"===f.platform.substring(0,3);(function(){function h(n){n=n||{};var w=Number(n.ports||n.port),x=w&&"port"||
"",A=f.root=n.webRoot?t(n.webRoot):{"*":"*"},D=f.extention+".html",G;var C=0;var r=[!1,["127.0.0.1","10.8.*.*"]];if(c)return a.log("Duplicated initialization ignored."),this;c=!0;f.version||(f.version="");f.password="public-defualt-8NJzmhSCpQutF7EY6E6WdxrRy9txtN93";f.salt="8NJzmhSCpQutF7EY6E6WdxrRy9txtN93";C=Math.floor(Number(C));f.timeZoneString=-12<C&&12>C?("+0"+String(C)).replace("0-","-0").replace("00","0").replace("010","10").replace("011","11").replace("+-","-")+":00":"+00:00";f.timeZoneOffset=
0<C<12?3600*C||0:0;if(x&&null===f.root)f.log("Invalid webRoot config.","e");else{f.unlocked=!0;f._locker=0;if(!A["*"]){C=Object.keys(A)[0];var u=A[C];if(u)A["*"]=u,a.log("no [*] sets, specify "+u+" (which hosts "+C+") as default webRoot.");else return f.log("Invalid config on webRoot.","c"),this}f.init=["",A,D,f.extention,n.tmp&&aa(n.tmp)||pa,null];f.tmpdir=f.init[4];f.logfile=f.init[5]=n.logfile&&path.normalize(n.logfile)||null;for(v of Aa)v();r[0]&&f.nextTick(function(){f.allow(r[1]);a.log("built-in firewall initialized;")});
for(G of Ba)G();va();wa();xa();ua();ya();W("sendET",!0);W("csBuilder",g);n._sendFileDir&&W("sendFileDir",n._sendFileDir);n._sendFileHeader&&W("sendFileHeader",n._sendFileHeader);n._proxySendFile&&W("proxySendFile",n._proxySendFile);var v=n._hotCodeDelaySeconds;"number"===typeof v&&(Infinity===v||/^[0-9]+$/.test(v.toString()))&&W("checkInterval",1E3*v);n._aliasHostHeader&&W("aliasHostHeader",n._aliasHostHeader);f.version||(f.version="suki.js_v11.0.21");"port"===x&&(ca.listen(w,n.bind||"::"),ca.parse=
oa,f.log(f.dateFormat(new Date)+" UTC\ndeno_v"+Deno.version.deno.replace("v","")+"\n"+f.version+" Javascript Application Server (HTTP listening on "+(n.bind||"::")+":"+w+")  \r\n"));return q}}function q(n){n(f)}function t(n){function w(G){var C="",r="",u="undefined"===typeof fetch;Array.isArray(G)&&("string"===typeof G[0]&&(C=G[0]),"string"===typeof G[1]&&(r=G[1]));"string"===typeof G&&(C=G);if("http://"===C.substring(0,7)||"https://"===C.substring(0,8)){if(u)return f.log("Invalid remote webRoot: "+
C+" (runtime has no fetch api so networking webRoot could not be used)","e"),null;"/"===C.slice(-1)&&(C=C.slice(0,-1));if(~C.indexOf(" "))return f.log("Invalid remote webRoot: "+C,"e"),null;r&&=aa(r);return C+" "+r}if(C)return C=aa(C)," "+C;f.log("Invalid webRoot: "+C,"e");return null}var x=n,A,D=!0;if("string"===typeof n||Array.isArray(n))x={"*":n};for(A in x)x[A]=w(x[A]),D&&x[A]&&(D=!1);return D?null:x}function g(n,w,x){n=n.trim();var A={},D=l[0].test(n),G={app:function(F){return F&&"string"===
typeof F?this.app=f._setAC(F,{_name:F}):null}},C=/\\/g;x="var __filename='"+(x||"").replace(C,"\\\\")+"',__dirname='"+(w||"").replace(C,"\\\\")+"';";var r;try{if(D){var u=n.match(l[1]);var v=u.length;var z=u.join("").replace(l[2],"");for(r=0;r<v;r++)n=n.replace(u[r],"");var B=x+"try{"+z+"\n}catch(e){return function (R){return R.error('CS AREA A HAS AN ERROR.\\n'+(e.stack||e),'500');};}return "+m+"function (R,tplVals,_get,_post,_cookie,_server){if(!suki.unlocked){return R.error('Server is too busy due to [Critical Error: uncaughtException(see logs)].','500.13');}try{"+
n+"\n}catch(e){R.error('CS AREA B HAS AN ERRROR.\\n'+(e.stack||e),500)};};"}else B=x+"return "+m+"function (R,tplVals,_get,_post,_cookie,_server){if(!suki.unlocked){return R.error('Server is too busy due to [Critical Error: uncaughtException(see logs)].','500.13');}try{"+n+"\n}catch(e){return R.error('CS AREA B HAS AN ERROR.\\n'+(e.stack||e),500);}};";A.cs=(new d("require,suki",B)).bind(G)(b(w),f)}catch(F){A.cs=function(y){y.error("["+f.extention.substring(1)+" init error] "+(F.stack||F),500)}}return A}
function b(n){function w(x){var A=!String(x).isAbs()&&!~x.indexOf("node:")&&(~x.indexOf("/")||~x.indexOf("\\"));return e(A?n+"/"+x:x,T)||e(n+"/"+x,T)}w.cache=T.cache;w.extensions=T.extensions;w.resolve=function(x){var A=!String(x).isAbs()&&!~x.indexOf("node:")&&(~x.indexOf("/")||~x.indexOf("\\"));try{var D=T.resolve(A?n+"/"+x:x)}catch(G){D=""}return D||T.resolve(n+"/"+x)};return w}function e(n,w){w||=T;try{var x=w(n)}catch(A){if(n.isAbs()||~n.indexOf("node:"))f.log("'libExists()' Pre Loading Failed: "+
n,"w"),console.log(A.stack||A)}return x}var c=!1,a={log:function(n){console.log(n)}},d=Object.getPrototypeOf(function(){}).constructor,m="async ";try{new d("(async function(){})")}catch(n){m=""}var l=[RegExp("\\/\\/\\<\\!\\-\\-([\\s\\S]*?)\\/\\/\\-\\-\\>"),RegExp("\\/\\/\\<\\!\\-\\-[\\s\\S]*?\\/\\/\\-\\-\\>","g"),RegExp("\\/\\/\\<\\!\\-\\-|\\/\\/\\-\\-\\>","g")],k,p=[];f.init=function(n){var w;void 0===k&&(k=n||{});setTimeout(function(){if(T&&k){h(k);k=null;for(w of p)w(f);p.length=0}},0);return function(x){"function"===
typeof x&&p.push(x)}}})();(function(){String.prototype.isAbs=function(){return"/"===this.charAt(0)||"\\"===this.charAt(0)||":"===this.charAt(1)};String.prototype.toContextString=function(){return JSON.stringify([this]).slice(1,-1)};var h=function(q){return"\\"+q};String.prototype.escapeMarkdown=function(q){var t="[\\.\\`\\*\\_\\{\\}\\[\\]\\<\\>\\(\\)\\#\\+\\-\\!\\|\\\\]",g;if(q){q=q.split("");var b=q.length;for(g=0;g<b;g++)t=t.replace("\\"+q[g],"")}return this.replace(new RegExp(t,"g"),h)}})();(function(){function h(c,
a,d){var m=a.replace(/set-cookie/g,"Set-Cookie"),l=encodeURIComponent(c)+"=",k=m.indexOf(l);if(~k){k+=l.length;var p=m.indexOf(";",k);~p||(p=m.length);a=m.substr(p);if(~a.indexOf(l))return h(c,a);try{var n=decodeURIComponent(m.substring(k,p))}catch{return}if(!0===d){p=m.indexOf("Set-Cookie",k);p=~p?p:m.length;a=m.substring(k+n.length,p);c={name:c,value:n};d=a.split(";");n=d.length;for(a=0;a<n;a++)m=d[a].trim(),""!==m&&(l=m.indexOf("="),~l?c[m.substr(0,l)]=m.substr(l+1):c[m]=!0);return c}return n}}
function q(c,a){return"~"+a.toUpperCase()+":"}function t(c,a){return"object"===typeof c?a?c:JSON.stringify(c,g,"\t"):"^"+typeof c+":\\{: "+String(c)}function g(c,a){return t(a,Array.isArray(a)?"^array:\\{: ":"^"+typeof a+":\\{: ")}f.objectToArray=function(c){return Array.prototype.slice.call(c,0)};f.next=new Function("...a","return a");var b=[/\\|%5c/gi,/&(?!\w+;)/g,/</g,/>/g,/"/g,/'/g,/%26/g,/%3c/gi,/%3e/gi,/&#37;22/g,/%/g];f.clean=function(c){if(!c)return 0===c?"0":"";c=String(c).replace(b[0],"").replace(b[1],
...
console.log("\r\n"+c+"\r\n")}};var e=[/: "\^([a-z]+):\\\\{:/g,/"(,?)$/gm];f.obj2str=function(c){try{var a="object"===typeof c?t(c).replace(e[0],q).replace(e[1],"$1"):"^"+(typeof c).toUpperCase()+": "+String(c)}catch{return util.inspect(c)}return a};f.parseJSON=function(c,a){try{return JSON.parse(c)}catch{return a||null}};f.signature=function(c,a){var d=(c=String(c)).substr(0,c.indexOf("."));a=a||"";if(64===d.length){var m=c.substr(65);return d===f.sha256(m+f.salt+a)?c:f.sha256(c+f.salt+a)+"."+c}return f.sha256(c+
f.salt+a)+"."+c};f.rewriteURL=function(c){var a=0,d=f.rewriteRules,m=d.length,l=c.indexOf("?"),k=~l?c.substr(l+1):"";for(k&&(c=c.substr(0,l));a<m;a++)if(l=d[a],l=c.replace(l[0],l[1]),l!==c)return l.substr(1)+(k?(~l.indexOf("?")?"&":"?")+k:"");return k?c+"?"+k:c}})();(function(){var h=require("node:fs"),q=require("node:dns");f.dnsTTL=3E5;f.dnsCache={_length:0};f.dnsHosts={};f.dnsInitHosts=function(t){t=Array.isArray(t)?t:h.readFileSync(t).toString("utf8").replace(/[\r\t]/g," ").replace(/[ ]+/g," ").split("\n");
...
else g(e);else q.resolve4(t,function(a,d){a?a=t:(a=d[0],b._length++,b[t]=[a,(new Date).getTime()]);if(c)g[0](a);else g(a)})}})();(function(){var h={};f.getAC=function(q){return h[q]||null};f.setAC=function(q,t){return"_"===q.charAt(0)?null:h[q]||(h[q]=t)};f._setAC=function(q,t){return h["_"+q]||(h["_"+q]=t)};f.resetAC=function(q,t){return h[q]=t}})();(function(){f.isAllowed=function(h,q,t,g){q=Array.isArray(q)?q:String(q).split(",");return q[h]||q[h.substr(0,h.indexOf(".",4))+".*.*"]||q[h.substr(0,
h.lastIndexOf("."))+".*"]||q[h.substr(0,h.indexOf("."))+".*.*.*"]?!1:t&&!!t.error(g||"Only valid agent can access.",404)||!0};f._defend=function(h,q){return q[h]||q[h.substr(0,h.indexOf(".",4))+".*.*"]||q[h.substr(0,h.lastIndexOf("."))+".*"]||q[h.substr(0,h.indexOf("."))+".*.*.*"]?!1:!0};f.allow=function(h){if(_unblocked){var q={};h=Array.isArray(h)?h:String(h).split(",");h.forEach(t=>q[t]=!0);_unblocked=!1;REST.config("_f",q)}};f.unblock=function(){_unblocked=!0;REST.config("_f",!1)}})();(function(){var h=
f.nullFunc,q=f.isWindows,t=f.isLinux||q,g={},b=require("node:child_process");f.shell=g;g.deamon=t?function(e,c,a){var d=e.cmd||e,m=Array.isArray(d)?d:q?String(d).split(" "):["sh","-c",String(d)],l=m.shift(),k=e.cwd?b.spawn(l,m,{cwd:e.cwd}):b.spawn(l,m),p=(e="boolean"==typeof c)?c:!1,n=e?a||5E3:"number"==typeof c?c:5E3,w=this,x=function(){this.resume&&(this.resume=h);k=b.spawn(l,m);k.on("exit",A);k.on("error",h)},A=function(D){0==D||p&&setTimeout(x,n)};k.on("exit",A);k.on("error",h);return{stop:function(D){p=
!1;w.kill(k);D||(this.stop=this.pause=this.resume=k=x=A=null)},pause:function(){this.stop(!0);this.resume=x}}}:h;g.exec=t?function(e,c,a){var d=e.cmd||e;d=Array.isArray(d)?d:q?String(d).split(" "):["sh","-c",String(d)];var m=e.cwd?b.spawn(d.shift(),d,{cwd:e.cwd}):b.spawn(d.shift(),d),l=this,k,p,n,w,x;a||=c;c=Number(c)||9E4;if(p=(k="object"===typeof a)||"function"===typeof a){var A=w="";e=function(G){x=!0;w+=G?G.toString("utf8"):"shell.exec error(no msg)"};m.stdout.on("data",function(G){A+=G.toString("utf8")});
m.stderr.on("data",e);m.on("error",e)}if(c){m.on("exit",function(G){if(!n){x||=0!=G;if(k)a[0](A||void 0,x?w:void 0);else p&&a(A||void 0,x?w:void 0);clearTimeout(D)}});var D=setTimeout(function(){if(!n){if(k)a[0](A||void 0,x?w+"[TIMEOUT("+c+"ms)]":"[TIMEOUT("+c+"ms)]");else p&&a(A||void 0,x?w+"[TIMEOUT("+c+"ms)]":"[TIMEOUT("+c+"ms)]");l.kill(m);n=!0}},c)}}:h;g.kill=t?function(e){var c="object"===typeof e?e.pid:"number"===typeof e&&e;q?b.spawn("taskkill",["/f","/t",c?"/pid":"/im",c||e]):b.spawn(c&&
...
null}},function(){var h=f.timeZoneOffset,q=f.timeZoneString,t=/[+\-][0-9]{2}:00$/;f.time=function(){return Math.floor((new Date).getTime()/1E3)+h};f.GMT=f.UTC=function(g){return void 0===g||-12<g&&12>g?Math.floor((new Date).getTime()/1E3)+3600*(g||0):Date.parse(t.test(String(g).trimRight())?g:g+q)/1E3||0};f.getGMT=f.getUTC=function(g){var b=new Date;g&&b.setTime(b.getTime()+1E3*(Number(g)||0));return b.toUTCString()};f.dateFormat=function(g,b){g||="{YYYY}-{MM}-{dd} {hh}:{mm}:{ss}";b||"string"!==typeof g?
...
0;clearTimeout(this.tid)}}function g(b){for(var e=0,c=b._q,a=b._q.length;e<a;e++)b.expel(c[e]);c.length=0}f.queue=function(){return{_q:[],_tid:0,_pendingTaskLength:0,_pendingTask:{},max:10,patience:3E4,moreTask:q,lessTask:t,add:h,dismiss:f.nullFunc,expel:f.nullFunc}}},function(){function h(g){for(var b="",e=q;0<g;)b=e[g%62]+b,g=Math.floor(g/62);return b||String(g)}var q="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".split(""),t=[,48,29,87];f.int62=h.encode=h;h.decode=function(g){for(var b=
String(g),e=0,c=0,a=b.charCodeAt(0);a;)e=62*e+a-t[a>>5],a=b.charCodeAt(++c);return g?e||NaN:NaN}},function(){var h=/MSIE ([^;]+)/;f.isSmartBrowser=function(b){return h.test("object"===typeof b?b["user-agent"]:b)?10>Number(RegExp.$1):!0};var q=/mobile|android|ucweb|fennec|iphone|blackberry|symbian|windows phone/i;f.isPhone=function(b){return q.test("object"===typeof b?b["user-agent"]:b)};f.long2ip=function(b){b=Number(b);return 12>String(b).length&&Math.floor(b)===b&&0<b?(b>>24&255)+"."+(b>>16&255)+
"."+(b>>8&255)+"."+(b&255):""};var t=/^[0-9]+$/;f.ip2long=function(b){b=String(b).split(".").filter(Boolean);return 4===b.length&&t.test(b.join(""))&&0<b[0]&&-1<b[1]&&-1<b[2]&&-1<b[3]&&256>b[0]&&256>b[1]&&256>b[2]&&256>b[3]?16777216*b[0]+65536*b[1]+256*b[2]+Number(b[3]):0};f.isIp4InCidrs=function(b,e){var c=f.ip2long;return Array.isArray(e)&&e.length&&e.some(function(a){var d=a.indexOf("/"),m=~(Math.pow(2,32-a.substr(d+1))-1);return~d?(c(b)&m)===(c(a.substr(0,d))&m):!1})||!1};f.ip4internals=[["10.0.0.0/8",
...
g[1].test(RegExp.$1)};f.random=function(b,e){var c="",a=Math.random;if(!b)return"";for(;;){var d=a().toString(36).substr(2);if(c.length+d.length>b){c+=d.substr(0,b-c.length);break}c+=d}if(e){e=Math.round;for(var m="",l=0;l<b;l++)d=c.charAt(l),m+=isNaN(Number(d))&&(e(a())?d.toUpperCase():d.toLowerCase())||d;return m}return c};f.hot=function(b){return{h:{},l:0,c:Number(b)||500,cache:function(e,c){var a;if(void 0===c||void 0===e)return e?this.h[e]:void 0;if(this.l>this.c){var d={};var m=0;for(a in this.h)void 0===
this.h[a]||++m&&(d[a]=this.h[a]);if(m===this.c)return;this.h=d;this.l=m}this.h[e]||this.l++;return this.h[e]=c},delete:function(e){return e&&(this.h[e]=void 0)}}}}];var Ba=[function(){var h=f.nullFunc,q=require("node:fs"),t=require("node:buffer").Buffer;f.write=function(g,b,e,c){c?q.writeFile(g,"string"===typeof b?t.from(b,c):b,function(a){var d="object"===typeof e;try{if(d)e[0](a?!1:!0,a);else e(a?!1:!0,a)}catch(m){(a=d&&e.r)?a.error("[file-callback-func-crit]\n"+(m.stack||m),500):f.log("[file-callback-func-crit]\n"+
(m.stack||m))}}):e?"object"===typeof e&&e[0]?q.writeFile(g,"string"===typeof b?t.from(b):b,function(a){try{e[0](a?!1:!0,a)}catch(d){e.r?e.r.error("[file-callback-func-crit]\n"+(d.stack||d),500):f.log("[file-callback-func-crit]\n"+(d.stack||d))}}):"function"===typeof e?q.writeFile(g,"string"===typeof b?t.from(b):b,function(a){try{e(a?!1:!0,a)}catch(d){f.log("[file-callback-func-crit]\n"+(d.stack||d))}}):q.writeFile(g,"string"===typeof b?t.from(b,e):b,h):q.writeFile(g,"string"===typeof b?t.from(b):
b,h)};f.read=function(g,b,e){q.readFile(g,function(c,a){var d="object"===typeof b;try{if(d)b[0](c?void 0:e?a.toString(e):a,c);else b(c?void 0:e?a.toString(e):a,c)}catch(m){(c=d&&b.r)&&c.error("[file-callback-func-crit]\n"+(m.stack||m),500)}})};f.writeMore=function(g,b,e,c){c?q.appendFile(g,"string"===typeof b?t.from(b,c):b,function(a){var d="object"===typeof e;try{if(d)e[0](a?!1:!0,a);else e(a?!1:!0,a)}catch(m){(a=d&&e.r)?a.error("[file-callback-func-crit]\n"+(m.stack||m),500):f.log("[file-callback-func-crit]\n"+
(m.stack||m))}}):e?"object"===typeof e&&e[0]?q.appendFile(g,"string"===typeof b?t.from(b):b,function(a){try{e[0](a?!1:!0,a)}catch(d){e.r?e.r.error("[file-callback-func-crit]\n"+(d.stack||d),500):f.log("[file-callback-func-crit]\n"+(d.stack||d))}}):"function"===typeof e?q.appendFile(g,"string"===typeof b?t.from(b):b,function(a){try{e(a?!1:!0,a)}catch(d){f.log("[file-callback-func-crit]\n"+(d.stack||d))}}):q.appendFile(g,"string"===typeof b?t.from(b,e):b,h):q.appendFile(g,"string"===typeof b?t.from(b):
b,h)};f.delete=function(g,b){q.unlink(g,function(e){var c="object"===typeof b;try{if(c)b[0](e?!1:!0,e);else"function"===typeof b&&b(e?!1:!0,e)}catch(a){(e=c&&b.r)?e.error("[file-callback-func-crit]\n"+(a.stack||a),500):f.log("[file-callback-func-crit]\n"+(a.stack||a))}})}},function(){function h(d,m,l,k){var p=!1,n,w,x,A;if(this[0]===h){var D=this[1];D.hostname=d;d=D;m=this[2];l=this[3];k=this[4]}D="https"===d.protocol.substr(0,5)?g:t;var G=(A=m&&"function"===typeof m[0])||"function"===typeof m;var C=
d.host;var r=C.indexOf(":");C=~r?C.substr(0,r):C;d.path||(d.path=d.pathname+(d.search||""));d.host=d.hostname;d.hostname=C;G?(r="object"===typeof l,k||!r&&(k=l),l=r&&l||f.passAgentHeaders()):l="object"===typeof m&&m||f.passAgentHeaders();G&&"identity"!==l["accept-encoding"]&&(l["accept-encoding"]="gzip, deflate");l.host||(l.host=C);d.headers=l;d=D.request(d,function(y){var E=y;if(n)if(z)if(200!=y.statusCode)z(404);else{E=n;var I=w;var L=x,J=L[2],M="1.1"===J,K=y.headers,U=I.hasOwnProperty("charset"),
...
Q+"\r\nServer: "+f.server+L[1]+"\r\nConnection: "+(M&&"keep-alive"||"close")+"\r\n\r\n")}catch(R){console.log("suki.get: proxyCache error\n"+(R.stack||R))}z("length",Number(y.headers["content-length"]));y.on("data",B);y.on("end",z)}else q(y,n,w,x),y.on("data",B);else G&&("gzip"===y.headers["content-encoding"]?(I=e.createGunzip(),I.on("error",u),y.pipe(I),y=I):"deflate"===y.headers["content-encoding"]&&(I=e.createInflate(),I.on("error",u),y.pipe(I),y=I),k&&y.setEncoding(k),y.on("data",B)),z&&(z.res=
E,y.on("end",z))});var u=function(y){var E;if(p)n&&(z?(z(404),console.log(y)):(y=w,y._r=x,y.error("proxy: destination network unreachable.",404)));else try{if(A)m[0](void 0,y.stack||y);else m(void 0,y.stack||y)}catch(I){(E=A&&m.r)?E.error("[get-_callback-func-crit]\n"+(I.stack||I),500):f.log("[get-_callback-func-crit]\n"+(I.stack||I),"e")}p=!0};d.on("error",u);if(G){var v="";var z=function(){if(!p){var y=z.res;y[200==y.statusCode?"data":"errorData"]=v;try{if(A)m[0](y);else m(y)}catch(E){(y=A&&m.r)?
y.error("[get-_callback-func-crit]\n"+(E.stack||E),500):f.log("[get-_callback-func-crit]\n"+(E.stack||E),"e")}}p=!0};var B=function(y){"string"===typeof y?v+=y:v=v?a.concat([v,y]):y}}else{var F=function(y){n=y._r[0];w=y;x=y._r;B=F.ed;y._file&&(z=F.ee);return!0};p=!0}d.end();return F}function q(d,m,l,k){var p=k[2],n="1.1"===p,w=d.headers,x=l.hasOwnProperty("charset"),A=w["content-type"].split(";"),D=w["content-length"],G=!D,C=w["set-cookie"],r=w.location,u=w["content-encoding"],v=w["cache-control:"],
z=w.etag,B=w.expires;l.hasOwnProperty("contentType")||(l.contentType=A[0].trim());!x&&A[1]&&(l.charset=A[1].trim());l.hasOwnProperty("status")||(l.status=d.statusCode);if(G)d.on("end",function(){m.end()});try{m.write("HTTP/"+p+" "+l.status+"\r\nDate: "+(w.date||(new Date).toUTCString())+"\r\nContent-Type: "+l.contentType+(x?"; charset="+l.charset:"")+(G?"":"\r\nContent-Length: "+D)+(r?"\r\nLocation: "+r:"")+(u?"\r\nContent-Encoding: "+u:"")+(C?"\r\nSet-Cookie: "+(Array.isArray(C)?C.join(""):C):"")+
...
!1);f.get=c=function(d,m,l,k){var p=b.parse(d);d=String(p.hostname||p.host&&p.host.split(":")[0]||"");var n,w;if(!d)throw"suki.get: invalid hostname";if(/^[0-9.]+$/.test(d)||~d.indexOf("["))return h(p,m,l,k);if((w=(n=f.dnsCache[d])&&n[0])&&f.dnsTTL>(new Date).getTime()-n[1])return p.hostname=w,h(p,m,l,k);m=f.next(h,p,m,l,k);f.dnsLookup4(d,m);return m};c.async=function(d,m,l){c(d,l,m)}},function(){function h(k,p,n,w,x){this[0]===h&&(p=this[1],p.hostname=k,k=p,p=this[2],n=this[3],w=this[4],x=this[5]);
p||="__=";var A="https"===k.protocol.substr(0,5),D=n&&"function"===typeof n[0],G=D||"function"===typeof n;A=A?g:t;var C=!1,r="object"===typeof p,u=r&&"------WebKitFormBoundary"+f.random(16),v=u?u+"--\r\n":"",z=k.host;var B=z.indexOf(":");z=~B?z.substr(0,B):z;k.path||(k.path=k.pathname+(k.search||""));k.host=k.hostname;k.hostname=z;G?(B="object"===typeof w,x||!B&&(x=w),w=B&&w||f.passAgentHeaders()):w="object"===typeof n&&n||f.passAgentHeaders();w["content-type"]=r?"multipart/form-data; boundary="+
u.substr(2):"{"===p.charAt(0)||"["===p.charAt(0)?"application/json":"application/x-www-form-urlencoded";w.host||(w.host=z);if(r){var F="",y=0,E=[],I=[];B=null;var L=0,J,M=function(H){"isBuffer"===H?K.write(E[L]):E[L].unpipe(K);++L<E.length?(K.write("\r\n"+I[L]),l.isBuffer(E[L])?M("isBuffer"):E[L].pipe(K,a)):K.end("\r\n"+v)};for(J in p)z=p[J],"string"===typeof z?F+=u+'\r\nContent-Disposition: form-data; name="'+J+'"\r\n\r\n'+z+"\r\n":Array.isArray(z)?F+=u+'\r\nContent-Disposition: form-data; name="'+
...
l.byteLength(F)+v.length}w["content-length"]=r?y:l.byteLength(p);G&&"identity"!==w["accept-encoding"]&&(w["accept-encoding"]="gzip, deflate");k.headers=w;k.method="POST";var K=A.request(k,function(H){var S=H;if(R)N?N(404):(q(H,R,P,V),H.on("data",Q));else{if(G){if("gzip"===H.headers["content-encoding"]){var X=c.createGunzip();X.on("error",U);H.pipe(X);H=X}else"deflate"===H.headers["content-encoding"]&&(X=c.createInflate(),X.on("error",U),H.pipe(X),H=X);x&&H.setEncoding(x);H.on("data",Q)}N&&(N.res=
S,H.on("end",N))}});var U=function(H){if(C)R&&(N?N(404):(H=P,H._r=V,H.error("proxy: destination network unreachable.",404)));else try{if(D)n[0](void 0,H.stack||H);else n(void 0,H.stack||H)}catch(S){(H=D&&n.r)?H.error("[post-_callback-func-crit]\n"+(S.stack||S),500):f.log("[post-_callback-func-crit]\n"+(S.stack||S),"e")}C=!0};K.on("error",U);if(G){var O="";var N=function(){if(!C){var H=N.res;H[200==H.statusCode?"data":"errorData"]=O;try{if(D)n[0](H);else n(H)}catch(S){(H=D&&n.r)?H.error("[post-_callback-func-crit]\n"+
...
r=D["content-length"],u=!r,v=D["set-cookie"],z=D.location,B=D["content-encoding"];n.hasOwnProperty("contentType")||(n.contentType=C[0].trim());!G&&C[1]&&(n.charset=C[1].trim());n.hasOwnProperty("status")||(n.status=k.statusCode);if(!n._file){if(u)k.on("end",function(){p.end()});try{p.write("HTTP/"+x+" "+n.status+"\r\nDate: "+(D.date||(new Date).toUTCString())+"\r\nContent-Type: "+n.contentType+(G?"; charset="+n.charset:"")+(u?"":"\r\nContent-Length: "+r)+(z?"\r\nLocation: "+z:"")+(B?"\r\nContent-Encoding: "+
B:"")+(v?"\r\nSet-Cookie: "+(Array.isArray(v)?v.join(""):v):"")+"\r\nCache-Control: must-revalidate, no-cache, private\r\nExpires: -1\r\nPragma: no-cache\r\nServer: "+(D.server||"Node.js")+w[1]+"\r\nConnection: "+(A&&"keep-alive"||"close")+"\r\n\r\n")}catch(F){console.log("suki.get: proxy error\n"+(F.stack||F))}}}var t=require("node:http"),g=require("node:https"),b=require("node:fs"),e=require("node:url"),c=require("node:zlib"),a={end:!1},d={flags:"r",encoding:null,fd:null,mode:438},m,l=require("node:buffer").Buffer;
t.globalAgent&&(t.globalAgent.maxSockets=g.globalAgent.maxSockets=Infinity,g.globalAgent.options.rejectUnauthorized=!1);f.post=m=function(k,p,n,w,x){var A=e.parse(k);k=String(A.hostname||A.host&&A.host.split(":")[0]||"");var D,G;if(!k)throw"suki.get: invalid hostname";if(/^[0-9.]+$/.test(k)||~k.indexOf("["))return h(A,p,n,w,x);if((G=(D=f.dnsCache[k])&&D[0])&&f.dnsTTL>(new Date).getTime()-D[1])return A.hostname=G,h(A,p,n,w,x);if(k)return p=f.next(h,A,p,n,w,x),f.dnsLookup4(k,p),p};m.async=function(k,
p,n,w){m(k,p,w,n)}},function(){f.binaryToHex=function(h){var q=h.length,t="",g=0;try{if(8<q){var b=Math.floor(q/8);var e=q%8;var c=0;if(0<e)for(;0<e;e--)t+=("0"+h.charCodeAt(c++).toString(16)).slice(-2);for(;0<b;b--)t+=("0"+h.charCodeAt(c).toString(16)).slice(-2)+("0"+h.charCodeAt(c+1).toString(16)).slice(-2)+("0"+h.charCodeAt(c+2).toString(16)).slice(-2)+("0"+h.charCodeAt(c+3).toString(16)).slice(-2)+("0"+h.charCodeAt(c+4).toString(16)).slice(-2)+("0"+h.charCodeAt(c+5).toString(16)).slice(-2)+("0"+
h.charCodeAt(c+6).toString(16)).slice(-2)+("0"+h.charCodeAt(c+7).toString(16)).slice(-2),c+=8}else for(;g<q;g++)t+=("0"+h.charCodeAt(g).toString(16)).slice(-2);return t}catch{}}},function(){f.binaryToUTF8=function(h){var q="",t=0,g=0,b=h.length,e=String.fromCharCode,c=0;try{for(;c<b;++c){var a=h.charCodeAt(c);128==(a&192)?0<g&&(t=t<<6|a&63,--g):(0==g&&(q+=e(t)),128>a?(t=a,g=0):192==(a&224)?(t=a&31,g=1):224==(a&240)?(t=a&15,g=2):240==(a&248)&&(t=a&7,g=3))}0==g&&(q+=e(t));return q.substring(1)}catch{}}},
function(){f.bufferToBinary=function(h){h=h.length;var q="",t=String.fromCharCode,g=0;try{if(8<h){var b=Math.floor(h/8);var e=h%8;var c=0;if(0<e)for(;0<e;e--)q+=t(data[c++]);for(;0<b;b--)q+=t(data[c],data[c+1],data[c+2],data[c+3],data[c+4],data[c+5],data[c+6],data[c+7]),c+=8}else for(;g<h;g++)q+=t(data[g]);return q}catch{}}}]})(suki,node_modules);
